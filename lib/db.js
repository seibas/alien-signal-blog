import { Redis } from '@upstash/redis';

const POSTS_KEY = 'blog:posts';

let redisInstance = null;

function getRedis() {
  if (redisInstance) {
    return redisInstance;
  }
  
  // Try different possible environment variable names
  let url = process.env.UPSTASH_REDIS_REST_URL || 
            process.env.UPSTASH_REDIS_REST_KV_URL ||
            process.env.KV_REST_API_URL;
              
  let token = process.env.UPSTASH_REDIS_REST_TOKEN || 
              process.env.UPSTASH_REDIS_REST_KV_REST_API_TOKEN ||
              process.env.KV_REST_API_TOKEN;
  
  // If we got a rediss:// URL instead of https://, convert it
  if (!url || url.startsWith('rediss://') || url.startsWith('redis://')) {
    // Parse the Redis URL to extract host and password
    const redisUrl = url || process.env.UPSTASH_REDIS_REST_KV_URL || process.env.REDIS_URL;
    if (redisUrl) {
      try {
        const parsedUrl = new URL(redisUrl);
        url = `https://${parsedUrl.hostname}`;
        token = parsedUrl.password || token;
      } catch (e) {
        throw new Error(`Failed to parse Redis URL: ${e.message}`);
      }
    }
  }
  
  if (!url || !token) {
    throw new Error(`Missing Redis credentials. URL: ${url}, Token: ${token ? 'set' : 'missing'}`);
  }
  
  redisInstance = new Redis({ url, token });
  return redisInstance;
}

// Get all posts
export async function getAllPosts() {
  try {
    const redis = getRedis();
    const posts = await redis.get(POSTS_KEY);
    if (!posts || !Array.isArray(posts)) {
      return [];
    }
    return posts.sort((a, b) => (a.date < b.date ? 1 : -1));
  } catch (error) {
    console.error('Error getting posts:', error);
    return [];
  }
}

// Get a single post by slug
export async function getPostBySlug(slug) {
  try {
    const redis = getRedis();
    const posts = await redis.get(POSTS_KEY);
    if (!posts || !Array.isArray(posts)) {
      return null;
    }
    return posts.find((p) => p.slug === slug) || null;
  } catch (error) {
    console.error('Error getting post:', error);
    return null;
  }
}

// Create a new post
export async function createPost(postData) {
  try {
    const redis = getRedis();
    const posts = await redis.get(POSTS_KEY) || [];
    
    // Check if slug already exists
    if (posts.find(p => p.slug === postData.slug)) {
      throw new Error('A post with this slug already exists');
    }

    // Add new post
    posts.push(postData);
    
    // Save back to Redis
    await redis.set(POSTS_KEY, posts);
    
    return { success: true, post: postData };
  } catch (error) {
    console.error('Error creating post:', error);
    throw error;
  }
}

// Update an existing post
export async function updatePost(slug, postData) {
  try {
    const redis = getRedis();
    const posts = await redis.get(POSTS_KEY) || [];
    
    // Find the post index by the OLD slug
    const postIndex = posts.findIndex(p => p.slug === slug);
    if (postIndex === -1) {
      throw new Error('Post not found');
    }

    // Merge the old post data with new data (preserve all fields)
    posts[postIndex] = { 
      ...posts[postIndex],  // Keep all existing fields
      ...postData           // Override with new data
    };
    
    // Save back to Redis
    await redis.set(POSTS_KEY, posts);
    
    return { success: true, post: posts[postIndex] };
  } catch (error) {
    console.error('Error updating post:', error);
    throw error;
  }
}

// Delete a post
export async function deletePost(slug) {
  try {
    const redis = getRedis();
    const posts = await redis.get(POSTS_KEY) || [];
    
    // Filter out the post
    const filteredPosts = posts.filter(p => p.slug !== slug);
    
    if (filteredPosts.length === posts.length) {
      throw new Error('Post not found');
    }
    
    // Save back to Redis
    await redis.set(POSTS_KEY, filteredPosts);
    
    return { success: true };
  } catch (error) {
    console.error('Error deleting post:', error);
    throw error;
  }
}

// Initialize posts (for migration)
export async function initializePosts(posts) {
  try {
    const redis = getRedis();
    await redis.set(POSTS_KEY, posts);
    return { success: true };
  } catch (error) {
    console.error('Error initializing posts:', error);
    throw error;
  }
}
